...
def draw_use_case_diagram():
    # Kích thước lớn, độ phân giải cao 300 DPI
    ...
    ax.text(50, 98, "SƠ ĐỒ USE CASE CHI TIẾT HỆ THỐNG QUẢN TRỊ BÁN LẺ CHỐNG LÃNG PHÍ", 
            ha='center', va='center', fontsize=22, fontweight='bold', color='#1E3A8A', fontfamily='sans-serif')
    ax.text(50, 96, "Mô hình phân quyền Store Staff & Store Manager | Chuỗi quy trình nghiệp vụ tác nghiệp khép kín", 
            ha='center', va='center', fontsize=14, fontstyle='italic', color='#475569', fontfamily='sans-serif')
    ...
    ax.text(50, 92.7, "HỆ THỐNG QUẢN TRỊ CHUỖI CUNG ỨNG BÁN LẺ (RETAIL SPOILAGE PREDICTOR)", 
    ...
    def draw_actor(x, y, label, role_desc, color='#1E40AF'):
        ...
        ax.text(x, y - 5.2, label, ha='center', va='center', fontsize=12, fontweight='bold', color=color)
    ...
    # Vẽ Actor Kho tổng DC (Secondary Actor ở góc trên bên phải)
    ...
    def draw_uc(x, y, w, h, text, uc_id="", bg_color='#F0FDF4', border_color='#16A34A', is_sub=False):
        ...
        if uc_id:
            ax.text(x, y + (h * 0.22), f"«{uc_id}»", ha='center', va='center', fontsize=8.5, 
                    fontweight='bold', color='#64748B', zorder=4)
            ax.text(x, y - (h * 0.08), text, ha='center', va='center', fontsize=10, 
    ...
    # =========================================================================
    # CÁC CỤM PHÂN HỆ NGHIỆP VỤ (VẼ HỘP NHÓM & CÁC USE CASE)
    # =========================================================================

    # --- KHỐI 1: QUẢN LÝ TIẾP NHẬN & NHẬP LÔ HÀNG (Yêu cầu chính của thầy Nhật) ---
    ...
    # Sub-UC 1.2: Kiểm đếm số lượng & HSD thực tế
    draw_uc(42, 85.5, 14, 3.8, "Kiểm đếm SL &\nCheck HSD thực tế", "UC-01.1", '#F0FDFA', '#14B8A6', is_sub=True)
    # Sub-UC 1.3: Nhập thông tin lô & Kích hoạt Shelf-Life
    draw_uc(42, 79.5, 14, 4.0, "Nhập thông tin lô &\nKích hoạt Shelf-Life", "UC-01.2", '#F0FDFA', '#14B8A6', is_sub=True)
    # Sub-UC 1.4: In tem nhãn kệ / Barcode lô
    draw_uc(28, 76.5, 13, 3.5, "In tem nhãn kệ /\nMã vạch lô", "UC-01.3", '#F0FDFA', '#14B8A6', is_sub=True)
    ...
    # --- KHỐI 2: BÁN HÀNG LẺ & TRỪ TỒN KHO FEFO ---
    ...
    # Sub-UC 2.3: Tự động trừ tồn kho lô
    draw_uc(42, 60.5, 14, 3.8, "Tự động trừ\ntồn kho của lô", "UC-02.2", '#FFF1F2', '#F43F5E', is_sub=True)
    # Sub-UC 2.4: Ghi nhận doanh số & Biến thời tiết/Lễ
    draw_uc(30, 55.5, 16, 4.0, "Lưu lịch sử bán &\nBiến thời tiết, Lễ", "UC-02.3", '#FFF1F2', '#F43F5E', is_sub=True)
    ...
    # --- KHỐI 3: THEO DÕI HSD & CẢNH BÁO SPOILAGE ---
    ...
    ax.add_patch(box3)
    ax.text(21, 48.5, "PHÂN HỆ 3: THEO DÕI HSD & CẢNH BÁO SPOILAGE", fontsize=10.5, fontweight='bold', color='#A16207')

    # UC 3.1: Theo dõi HSD & Nhận cảnh báo đa cấp
    draw_uc(30, 42.5, 16, 4.5, "Giám sát HSD &\nCảnh báo Spoilage", "UC-03", '#FEF08A', '#CA8A04')
    ...
    # Sub-UC 3.4: Đảo hàng ra quầy & Dán tem giảm giá
    draw_uc(28, 33.5, 14, 4.0, "Đảo hàng ra trước quầy\n& Dán tem giảm giá", "UC-03.3", '#FEFCE8', '#EAB308', is_sub=True)
    # Sub-UC 3.5: Khóa mã vạch khi hết hạn
    draw_uc(42, 30.5, 14, 3.8, "Khóa mã vạch POS\n(Khi hết hạn <= 0 ngày)", "UC-03.4", '#FEFCE8', '#EAB308', is_sub=True)
    ...
    # --- KHỐI 4: QUY TRÌNH TIÊU HỦY HÀNG HỎNG / QUÁ DATE ---
    ...
    # UC 4.1: Lập phiếu tiêu hủy hàng hỏng
    draw_uc(28, 16.5, 14, 4.5, "Lập phiếu yêu cầu\ntiêu hủy hàng hỏng", "UC-04", '#F3E8FF', '#9333EA')
    # UC 4.2: Phê duyệt tiêu hủy (Manager)
    draw_uc(42, 16.5, 14, 4.5, "Phê duyệt biên bản\ntiêu hủy hàng", "UC-05", '#F3E8FF', '#9333EA')
    # Sub-UC 4.3: Trừ sạch tồn kho của lô về 0
    draw_uc(30, 9.5, 15, 3.8, "Trừ sạch tồn kho\n(status = 'DISPOSED')", "UC-04.1", '#FAF5FF', '#A855F7', is_sub=True)
    # Sub-UC 4.4: Hạch toán tổn thất tài chính
    draw_uc(43, 9.5, 14, 4.0, "Hạch toán thiệt hại &\nGhi sổ Spoilage Log", "UC-04.2", '#FAF5FF', '#A855F7', is_sub=True)

    # Quan hệ Khối 4
    draw_actor_link(14, 58, 22, 17.5, color='#9333EA') # Staff -> UC-04
    draw_actor_link(86, 58, 48, 17.5, color='#9333EA') # Manager -> UC-05
    draw_relation(35, 16.5, 36, 16.5, "<<transfer>>", color='#9333EA')
    draw_relation(42, 14.2, 34, 11.4, "<<include>>", color='#9333EA')
    draw_relation(42, 14.2, 43, 11.5, "<<include>>", color='#9333EA')

    # --- KHỐI 5: DỰ BÁO NHU CẦU & ĐỀ XUẤT ĐẶT HÀNG ---
    ...
    # UC 5.1: Phân tích lịch sử bán & Biến ngoại cảnh K
    draw_uc(66, 73.5, 16, 4.2, "Phân tích tốc độ bán (d)\n& Hệ số ngoại cảnh (K)", "UC-06", '#DBEAFE', '#2563EB')
    # UC 5.2: Tính toán ROP & Cảnh báo tồn kho nguy hiểm
    draw_uc(66, 66.5, 16, 4.2, "Tính Reorder Point (ROP)\n& Đánh giá DOS > DUE", "UC-07", '#DBEAFE', '#2563EB')
    # UC 5.3: Tự động sinh đề xuất đặt hàng tối ưu
    draw_uc(66, 59.5, 16, 4.2, "Tự động sinh Đề xuất\nđặt hàng tối ưu", "UC-08", '#DBEAFE', '#2563EB')
    # UC 5.4: Duyệt và gửi đơn đặt hàng về Kho tổng
    draw_uc(66, 54.5, 16, 4.0, "Duyệt & Gửi đơn đặt\nvề Kho tổng (DC)", "UC-09", '#DBEAFE', '#2563EB')
    ...
    # --- KHỐI 6: QUẢN TRỊ HỆ THỐNG & DASHBOARD THỐNG KÊ ---
    ...
    # UC 6.2: Import Excel lịch sử bán hàng
    draw_uc(66, 36.5, 16, 4.5, "Import file Excel bán lẻ\n& Đồng bộ dữ liệu POS", "UC-11", '#E2E8F0', '#475569')
    # UC 6.3: Xem Dashboard phân tích & Tỷ lệ hao hụt
    draw_uc(66, 29.5, 16, 4.5, "Xem Dashboard trực quan\n& Tỷ lệ Spoilage Loss", "UC-12", '#E2E8F0', '#475569')
    # UC 6.4: Xuất báo cáo kiểm kê định kỳ
    draw_uc(66, 24.5, 15, 3.5, "Xuất báo cáo kiểm kê\n& Chống lãng phí", "UC-13", '#F1F5F9', '#64748B', is_sub=True)
